...
def train_svm_from_files(file_path, test_size=0.8, kernel='rbf', C=1, gamma='scale'):
    ...
    sns.heatmap(cm_normalized, annot=True, fmt='.2f', cmap='Blues', xticklabels=[1, 2, 3, 4, 5, 6],
...
def train_and_save_svm(file_paths, model_save_path, kernel='rbf', C=1, gamma='scale'):
    merged_data = pd.read_csv(file_path)
...
# 함수 실행
# train_svm_from_files(file_path)
train_and_save_svm(file_path, model_save_path)