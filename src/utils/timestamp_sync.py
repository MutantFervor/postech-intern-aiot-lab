...
def interpolation(df, columns):
    for column in columns:
        ...
        df[column].ffill()  # 앞쪽 NaN 처리
        df[column].bfill() # 뒤쪽 NaN 처리
        ...
        # IQR 기반의 상한선과 하한선 설정 (범위를 넓게 설정)
        lower_bound = Q1 - 300.0 * IQR
        upper_bound = Q3 + 300.0 * IQR

        # IQR 범위를 벗어난 값을 NaN으로 처리
